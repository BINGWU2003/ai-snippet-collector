...
| 💬 **附加 AI 说明**        | 保存时可选输入一条说明（如"请修复此函数的 bug"），随引用一起发给 AI |
| 📋 **一键编译并复制**      | 状态栏按钮或 `Ctrl+Alt+C`，编译当前文件并复制到剪贴板，无需终端    |
...
| 编译并复制到剪贴板       | `Ctrl+Alt+C`       | `Cmd+Alt+C`       |

保存后弹出可选的说明输入框（按 Escape 跳过）：

```
添加说明给 AI（可选，按 Escape 跳过）
> 例如：请修复这个函数中的并发 bug
```
...
收集完毕后，点击右下角状态栏的 `📋` 图标（悬停显示目标文件名），或按 `Ctrl+Alt+C`，内容自动编译并写入剪贴板，粘贴到任意 AI 对话框即可。
...
├── statusBar.ts           # 状态栏（含编译复制按钮）
├── formatter.ts           # 引用格式化
├── anchorResolver.ts      # 锚点解析（行号漂移修正）
├── compiler.ts            # 引用展开核心逻辑（供扩展和 CLI 共用）
├── fileManager.ts         # 文件读写（含去重检测）
